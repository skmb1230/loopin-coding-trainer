...
왼쪽 **선택 학습 → 회의 용어**에서 원할 때만 시작합니다. 코딩테스트가 메인이며, 용어 학습은 Today·코테 커리큘럼·학습 시간 배분·레벨·연속 학습 기록에 포함되지 않습니다. 메뉴를 열기 전에는 용어 학습 화면과 기록을 불러오지 않습니다. 컴퓨터 기초, 비즈니스, 기획, 영업, 회의·판교어를 프로그래밍 언어와 관계없이 별도 진도로 학습합니다.
...
매일 해야 하는 과제가 아닙니다. 복습 날짜가 지나도 코테에 알림이나 미완료 과제가 생기지 않으며, 연습실을 다시 열었을 때만 추천에 사용합니다. Settings에서 **회의 용어 기록만 초기화**할 수 있고, 코테·이론 학습 기록 초기화는 회의 용어 기록을 지우지 않습니다.
