...
function Retrospective({ problem, onDone }) {
  ...
  return <div className="modal-backdrop"><section className="modal retrospective" role="dialog" aria-modal="true"><div className="success-orbit">✓</div><span className="eyebrow">SOLVED · 짧은 복기</span><h2>좋아요. 이제 풀이를<br />내 것으로 만들 차례예요.</h2><label>이 문제의 핵심 아이디어는?<textarea value={idea} onChange={(event)=>setIdea(event.target.value)} placeholder="예: 이미 본 값을 Set에 저장했다." /></label><fieldset><legend>시간복잡도는?</legend><div className="complexity-grid">{['O(1)','O(log N)','O(N)','O(N log N)','O(N²)'].map((item)=><button type="button" className={complexity===item?'selected':''} onClick={()=>setComplexity(item)} key={item}>{item}</button>)}</div></fieldset><label>비슷한 문제를 다시 만나면 어떤 신호를 볼까요?<textarea value={signal} onChange={(event)=>setSignal(event.target.value)} placeholder="예: 같은 값을 여러 번 찾고 있는지 본다." /></label><button disabled={!ready} className="primary-button full" onClick={()=>onDone({idea,complexity,signal})}>복기 저장하고 다음 문제 <span>→</span></button></section></div>;
}
