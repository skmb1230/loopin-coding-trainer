...
import { theoryTopics, aiTopics, careerTopics } from '../data/learningContent.js';
...
function LearningPage({ type }) {
  const config=type==='AI'
    ...
    : type==='Career'
      ? { topics:careerTopics, eyebrow:'INTERVIEW SUPPORT · 5%', title:'Git · AWS', description:'코테와 JavaScript 학습을 해치지 않는 범위에서 자주 나오는 실무 면접 개념을 익힙니다.' }
      : { topics:theoryTopics, eyebrow:'DEVELOPER THEORY', title:'이론 학습', description:'경력에 맞는 깊이로 JavaScript·브라우저·React를 다시 연결합니다.' };
  const topics=config.topics; const [active,setActive]=useState(topics[0]); const [reveal,setReveal]=useState(0);
  useEffect(()=>{setActive(topics[0]);setReveal(0);},[type]);
...
export default function App() {
  ...
  if(active==='career')page=<LearningPage type="Career"/>;
