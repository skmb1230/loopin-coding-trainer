...
import { navItems, navSections } from './navigation.js';
...
function Sidebar({ active, onNavigate, profile, languageId, currentLevel }) {
  const renderItem = ([id, icon, label]) => <button key={id} className={active === id ? 'active' : ''} onClick={() => onNavigate(id)} aria-label={label} aria-current={active === id ? 'page' : undefined} title={label}><span className="nav-item-icon" aria-hidden="true">{icon}</span><span className="nav-item-label">{label}</span></button>;
  return <aside className="sidebar">
    <div className="sidebar-top"><button className="brand" onClick={() => onNavigate('today')} aria-label="loopin 오늘의 학습"><span className="brand-mark">L</span><b>loopin</b></button><span className="sidebar-caption">학습 메뉴</span></div>
    <nav aria-label="학습 메뉴">
      {navSections.map((section) => <div className="nav-section" key={section.label}><div className="nav-label">{section.label}</div>{navItems.filter(([id]) => section.ids.includes(id)).map(renderItem)}</div>)}
    </nav>
    <button className="profile-mini" onClick={() => onNavigate('profile')} aria-label="내 정보 보기"><span>{getLanguage(languageId).label.slice(0,1)}</span><div><strong>내 정보 보기 →</strong><small>{getLanguage(languageId).label} · 레벨 {currentLevel}</small></div></button>
  </aside>;
}
...
function LearningPage({ type, languageId }) {
  const config=type==='AI'
    ? { topics:aiTopics, eyebrow:'FRONT-END × AI', title:'인공지능 학습', description:'도구 사용법보다 AI 기능을 안전하게 제품에 넣는 법을 연습합니다.' }
    : type==='Systems'
    ...
    : type==='Career'
      ? { topics:careerTopics, eyebrow:'INTERVIEW SUPPORT · 5%', title:'깃·클라우드 면접', description:`코테와 ${getLanguage(languageId).label} 학습을 해치지 않는 범위에서 자주 나오는 실무 면접 개념을 익힙니다.` }
      : languageId === 'java'
...
function SettingsPage({ settings, onSetting, onReload }) {
  ...
  return <div className="page settings-page"><PageHeader eyebrow="PREFERENCES & DATA" title="설정" description="집중 환경과 로컬 학습 데이터를 관리합니다." />{dataError&&<p className="data-action-error" role="alert">{dataError}</p>}{busy&&<p role="status">데이터 작업 중이에요. 완료될 때까지 기다려 주세요.</p>}<section><h2>학습 언어</h2><div className="setting-row"><div><strong>기본 풀이 언어</strong><p>오늘 학습·문제·이론·복습 기록이 선택한 언어로 전환됩니다.</p></div><select value={settings.learningLanguage} onChange={(event)=>onSetting('learningLanguage',event.target.value)}>{getAvailableLanguages().map((language)=><option value={language.id} key={language.id}>{language.label}</option>)}</select></div>{settings.learningLanguage==='java'&&<JavaRuntimeStatus />}</section><section><h2>화면과 에디터</h2><div className="setting-row"><div><strong>화면 테마</strong><p>오래 봐도 편안한 화면을 선택하세요.</p></div><div className="toggle-group"><button className={settings.theme==='light'?'active':''} onClick={()=>onSetting('theme','light')}>밝은 화면</button><button className={settings.theme==='dark'?'active':''} onClick={()=>onSetting('theme','dark')}>어두운 화면</button></div></div><div className="setting-row"><div><strong>에디터 글자 크기</strong><p>코드 에디터에만 적용됩니다.</p></div><select value={settings.editorFontSize} onChange={(event)=>onSetting('editorFontSize',Number(event.target.value))}>{[13,14,15,16,18].map((item)=><option value={item} key={item}>{item}px</option>)}</select></div><div className="setting-row"><div><strong>기본 집중 세션</strong><p>오늘의 학습 세션을 나누는 기준입니다.</p></div><select value={settings.focusMinutes} onChange={(event)=>onSetting('focusMinutes',Number(event.target.value))}>{[25,40,50,60,90].map((item)=><option value={item} key={item}>{item}분</option>)}</select></div></section><section><h2>백업과 복원</h2><div className="setting-row"><div><strong>데이터 내보내기</strong><p>진도, 코드, 메모, 이론과 회의 용어의 계획·복습 기록을 JSON 파일로 저장합니다.</p></div><button className="secondary-button" disabled={busy} onClick={exportData}>JSON 내보내기</button></div><div className="setting-row"><div><strong>데이터 가져오기</strong><p>다른 PC에서 저장한 loopin 백업을 복구합니다.</p></div><label className="secondary-button file-button">JSON 가져오기<input type="file" accept="application/json" disabled={busy} onChange={importData} /></label></div></section><section className="danger-zone"><h2>기록 초기화</h2><div className="setting-row"><div><strong>특정 Level 초기화</strong><p>선택한 레벨의 진도·코드·메모만 지웁니다.</p></div><div className="inline-reset"><select value={resetLevel} onChange={(event)=>setResetLevel(Number(event.target.value))}>{problemCountsByLevel.map((_,level)=><option value={level} key={level}>Level {level}</option>)}</select><button onClick={clearLevel}>선택 Level 초기화</button></div></div><div className="reset-actions"><button onClick={()=>reset('progress','문제 진행 상황')}>문제 진행 초기화</button><button disabled={busy} onClick={resetLearningHistory}>코테·이론 학습 기록 초기화</button><button disabled={busy} onClick={resetWords}>회의 용어 기록만 초기화</button><button onClick={()=>reset('code','작성 코드')}>문제 코드 초기화</button><button disabled={busy} onClick={resetAll}>전체 데이터 초기화</button></div></section></div>;
}
...
export default function App() {
  ...
  if(!profile&&!['words','profile','settings'].includes(active))return <Onboarding onComplete={completeOnboarding}/>;
  if(profile&&profile.onboardingResultSeen!==true&&!['words','profile','settings'].includes(active))return <OnboardingResult profile={profile} onComplete={completeOnboarding}/>;
  ...
  if(active==='profile')page=<Suspense fallback={<div className="app-loading"><p>내 정보를 불러오는 중이에요.</p></div>}><ProfilePage profile={profile} settings={settings} progress={progress} todayMinutes={profile&&!needsDailyTime?todayMinutes:null} onNavigate={navigate}/></Suspense>;
  if(active==='settings')page=<SettingsPage settings={settings} onSetting={onSetting} onReload={()=>window.location.reload()}/>;
  return <><div className="app-shell"><Sidebar active={active} onNavigate={navigate} profile={profile} languageId={settings.learningLanguage} currentLevel={curriculum.currentLevel}/><div className="app-content">{(dailyError||settingsError)&&<div className="app-save-alert" role="alert">{dailyError||settingsError}</div>}{page}</div></div>{needsLanguageChoice&&!['words','profile','settings'].includes(active)?<LanguageStartSetup selectedLanguage={settings.learningLanguage} onConfirm={confirmStartLanguage}/>:active==='today'&&needsDailyTime&&<DailyTimeSetup defaultMinutes={todayMinutes} onConfirm={changeTodayMinutes}/>}</>;
}
