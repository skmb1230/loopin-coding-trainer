...
import { calculateStudyStreak, deriveCurriculumState, getLevelsReferencedByIds, getLevelsReferencedByProgress, getProblemIdFromStorageKey, getProblemLevel, problemCountsByLevel } from '../core/curriculum/curriculumEngine.js';
...
const navItems = [
  ...
  ['theory', '◈', 'Theory'], ['ai', '✦', 'AI'], ['systems', '◎', 'Network · Security'], ['career', '◇', 'Git · AWS'], ['notes', '≡', 'Notes'], ['analytics', '⌗', 'Analytics'], ['settings', '⚙', 'Settings'],
];
...
const languageChoiceKey = () => `loopin-language-choice:${new Date().toLocaleDateString('en-CA')}`;
...
function TodayPage({ profile, daily, todayMinutes, onTimeChange, progress, mastery, onOpen, settings, languageId, curriculum, completedSessions, onSessionToggle }) {
  ...
  const sessions = useMemo(() => buildStudySessions(allocation, profile.focusMinutes || settings.focusMinutes, new Date(), languageLabel).map((session)=>({...session,done:completedSessions.includes(session.id)})), [allocation, completedSessions, languageLabel, profile.focusMinutes, settings.focusMinutes]);
...
function SettingsPage({ settings, onSetting, onReload }) {
  ...
  const resetLearningHistory=async()=>{if(!window.confirm('이론 학습 기록과 오늘 세션 완료 상태를 초기화할까요? 이 작업은 되돌릴 수 없어요.'))return;await Promise.all([storage.clear('sessions'),storage.clear('curriculum')]);localStorage.removeItem('loopin-learning-progress');onReload();};
...
export default function App() {
  ...
  const loadData=async(activeProfile=profile, languageOverride)=>{
    setLoading(true);
    const languageId = languageOverride || activeProfile?.learningLanguage || settings.learningLanguage || 'javascript';
    const [storedProgress,storedNotes,languageDaily,legacyDaily] = await Promise.all([
      storage.getAll('progress'), storage.getAll('notes'), storage.get('curriculum',todayKey(languageId)),
  ...
  const completeOnboarding=async(nextProfile)=>{sessionStorage.setItem(languageChoiceKey(),'done');setNeedsLanguageChoice(false);setProfile(nextProfile);setSettings((current)=>({...current,learningLanguage:nextProfile.learningLanguage||'javascript'}));setLocal('loopin-profile',nextProfile);await loadData(nextProfile);};
  const updateProgress=async(id,patch,languageId=settings.learningLanguage)=>{const key=getProgressStorageKey(id,languageId);setProgress((current)=>{const legacy=languageId==='javascript'?current[id]:undefined;const record={...(legacy||{}),...current[key],...patch,problemId:id,level:getProblemLevel(id),language:languageId};const next={...current,[key]:record};storage.set('progress',key,record);return next;});};
  const saveDaily=async({minutes=todayMinutes,plan=dailyPlan,sessions=dailySessionDone}={})=>storage.set('curriculum',todayKey(settings.learningLanguage),{minutes,ids:plan.map((item)=>item.id),completedSessions:sessions,level:curriculum.currentLevel,targetDifficulty:curriculum.targetDifficulty,updatedAt:new Date().toISOString()});
  const changeTodayMinutes=async(minutes)=>{const nextMinutes=Math.min(720,Math.max(30,Math.round(minutes)));const desired=dailyProblemCount(nextMinutes);const recommendations=selectDailyProblems({problems,progress:languageProgress,mastery,count:Math.max(desired,dailyPlan.length),difficulty:curriculum.targetDifficulty,targetLevel:curriculum.currentLevel});const completed=dailyPlan.filter((item)=>['SOLVED','SOLVED_WITH_HINT','MASTERED'].includes(languageProgress[item.id]?.status));const pending=dailyPlan.filter((item)=>!completed.some((done)=>done.id===item.id));const combined=[...completed,...pending,...recommendations].filter((item,index,array)=>array.findIndex((candidate)=>candidate.id===item.id)===index);const nextPlan=combined.slice(0,Math.max(desired,completed.length));setTodayMinutes(nextMinutes);setDailyPlan(nextPlan);setNeedsDailyTime(false);await saveDaily({minutes:nextMinutes,plan:nextPlan});};
  const toggleDailySession=(id)=>setDailySessionDone((current)=>{const next=current.includes(id)?current.filter((item)=>item!==id):[...current,id];saveDaily({sessions:next});return next;});
  ...
  let page;
  if(active==='today')page=<TodayPage profile={profile} daily={dailyPlan} todayMinutes={todayMinutes} onTimeChange={changeTodayMinutes} progress={languageProgress} mastery={mastery} onOpen={setSelected} settings={settings} languageId={settings.learningLanguage} curriculum={curriculum} completedSessions={dailySessionDone} onSessionToggle={toggleDailySession}/>;
  ...
  return <><div className="app-shell"><Sidebar active={active} collapsed={collapsed} onToggle={()=>setCollapsed((value)=>!value)} onNavigate={navigate} profile={profile} languageId={settings.learningLanguage} currentLevel={curriculum.currentLevel}/><div className="app-content">{page}{active==='today'&&!needsDailyTime&&<div className="today-time-floating"><DailyTimeControl minutes={todayMinutes} onChange={changeTodayMinutes} compact/></div>}</div></div>{needsLanguageChoice?<LanguageStartSetup selectedLanguage={settings.learningLanguage} onConfirm={confirmStartLanguage}/>:active==='today'&&needsDailyTime&&<DailyTimeSetup defaultMinutes={todayMinutes} onConfirm={changeTodayMinutes}/>}</>;
}
