...
import { theoryTopics, aiTopics } from '../data/learningContent.js';
...
const navItems = [
  ...
  ['theory', '◈', 'Theory'], ['ai', '✦', 'AI'], ['notes', '≡', 'Notes'], ['analytics', '⌗', 'Analytics'], ['settings', '⚙', 'Settings'],
];
...
function TodayPage({ profile, daily, todayMinutes, onTimeChange, progress, mastery, onOpen, settings }) {
  ...
  const [sessions, setSessions] = useState(() => buildStudySessions(allocation, profile.focusMinutes || settings.focusMinutes));
  useEffect(() => setSessions(buildStudySessions(allocation, profile.focusMinutes || settings.focusMinutes)), [allocation, profile.focusMinutes, settings.focusMinutes]);
...
function ProblemsPage({ problems, progress, onOpen }) {
  ...
  const filtered=problems.filter((item)=>item.title.includes(query)||item.id.toLowerCase().includes(query.toLowerCase())).filter((item)=>category==='전체'||item.category===category).filter((item)=>status==='전체'||(progress[item.id]?.status||'NOT_STARTED')===status);
  return <div className="page"><PageHeader eyebrow="30 PROBLEMS · LEVEL 0" title="문제 탐색" description="문제 유형과 현재 상태를 기준으로 다음 연습을 고르세요." action={<div className="header-stat"><b>{Object.values(progress).filter((item)=>item.status?.startsWith('SOLVED')).length}</b><span>solved</span></div>} /><div className="filter-bar"><label className="search-box"><span>⌕</span><input value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="문제명 또는 ID 검색" /></label><label>Level<select><option>Level 0</option></select></label><label>Category<select value={category} onChange={(event)=>setCategory(event.target.value)}>{categories.map((item)=><option key={item}>{item}</option>)}</select></label><label>Status<select value={status} onChange={(event)=>setStatus(event.target.value)}><option>전체</option><option value="NOT_STARTED">시작 전</option><option value="TRYING">풀이 중</option><option value="SOLVED">해결</option><option value="SOLVED_WITH_HINT">힌트로 해결</option><option value="FAILED">복습 필요</option></select></label></div><div className="problem-table"><div className="table-head"><span>상태</span><span>번호</span><span>문제</span><span>유형</span><span>난이도</span><span>예상 시간</span><span>마지막 풀이</span></div>{filtered.map((problem)=>{const record=progress[problem.id];const state=record?.status||'NOT_STARTED';return <button className="table-row" key={problem.id} onClick={()=>onOpen(problem)}><span><i className={`table-status ${state.toLowerCase()}`}>{['SOLVED','MASTERED'].includes(state)?'✓':state==='FAILED'?'!':'·'}</i>{statusLabels[state]}</span><span>{problem.id}</span><span><b>{problem.title}</b><small>{problem.concepts.join(' · ')}</small></span><span>{problem.category}</span><span>{'●'.repeat(problem.difficulty)}<i>{'●'.repeat(5-problem.difficulty)}</i></span><span>{problem.estimatedMinutes}분</span><span>{record?.lastAttempt?new Intl.DateTimeFormat('ko-KR').format(new Date(record.lastAttempt)):'—'} <b>→</b></span></button>})}{!filtered.length&&<div className="empty-table">조건에 맞는 문제가 없어요.</div>}</div></div>;
}
...
function ReviewPage({ problems, progress, onOpen }) {
  const [filter,setFilter]=useState('전체'); const now=Date.now();
  const review=problems.filter((problem)=>{const item=progress[problem.id];return item&&(['FAILED','SOLVED_WITH_HINT','REVIEW'].includes(item.status)||item.nextReview);}).filter((problem)=>{const item=progress[problem.id];if(filter==='전체')return true;if(filter==='오답')return item.status==='FAILED';if(filter==='힌트 사용')return item.hintsUsed>0;if(filter==='복습 예정')return item.nextReview;return true;});
  return <div className="page"><PageHeader eyebrow="SPACED REPETITION" title="복습 큐" description="틀린 문제와 힌트로 해결한 문제를 잊기 전에 다시 만납니다." action={<div className="header-stat amber"><b>{review.filter((item)=>!progress[item.id].nextReview||new Date(progress[item.id].nextReview).getTime()<=now).length}</b><span>오늘 복습</span></div>} /><div className="segmented">{['전체','오답','힌트 사용','시간 초과','오래 걸림','복습 예정'].map((item)=><button className={filter===item?'active':''} key={item} onClick={()=>setFilter(item)}>{item}</button>)}</div><div className="review-list">{review.map((problem)=>{const item=progress[problem.id];const due=!item.nextReview||new Date(item.nextReview).getTime()<=now;return <button key={problem.id} onClick={()=>onOpen(problem)}><span className={`review-icon ${item.status==='FAILED'?'failed':'hinted'}`}>{item.status==='FAILED'?'!':'💡'}</span><div><small>{problem.id} · {problem.category}</small><strong>{problem.title}</strong><p>{item.failureType==='timeout'?'시간 초과 · 현재 방식의 반복 횟수를 확인해보세요.':item.hintsUsed?`힌트 ${item.hintsUsed}단계 사용 · 스스로 다시 설명해보세요.`:'경계 조건을 다시 확인해보세요.'}</p></div><aside><b>{due?'오늘 복습':'예정'}</b><span>{item.nextReview?new Intl.DateTimeFormat('ko-KR',{month:'short',day:'numeric'}).format(new Date(item.nextReview)):'지금'}</span></aside></button>})}{!review.length&&<div className="empty-state"><span>✓</span><h3>지금 밀린 복습이 없어요.</h3><p>오답이나 힌트를 사용한 문제가 생기면 1·3·7·14일 간격으로 이곳에 나타납니다.</p></div>}</div></div>;
}

function LearningPage({ type }) {
  const topics=type==='AI'?aiTopics:theoryTopics; const [active,setActive]=useState(topics[0]); const [reveal,setReveal]=useState(0);
  const choose=(topic)=>{setActive(topic);setReveal(0);};
  return <div className="page"><PageHeader eyebrow={type==='AI'?'FRONT-END × AI':'DEVELOPER THEORY'} title={type==='AI'?'AI 학습':'이론 학습'} description={type==='AI'?'도구 사용법보다 AI 기능을 안전하게 제품에 넣는 법을 연습합니다.':'경력에 맞는 깊이로 JavaScript·브라우저·React를 다시 연결합니다.'} /><section className="learning-layout"><aside className="topic-list">{topics.map((topic,index)=><button className={active.id===topic.id?'active':''} onClick={()=>choose(topic)} key={topic.id}><span>{String(index+1).padStart(2,'0')}</span><div><small>{topic.area}</small><strong>{topic.title}</strong><p>{topic.minutes}분 · 사고 질문 포함</p></div></button>)}</aside><article className="lesson"><div className="lesson-meta"><span>{active.area}</span><span>약 {active.minutes}분</span></div><h1>{active.title}</h1><p className="lesson-intro">설명을 읽기 전에 먼저 생각해보세요. 정답 여부보다 어떤 근거로 판단했는지가 중요합니다.</p><div className="thought-question"><span>QUESTION</span><h2>{active.question}</h2>{reveal===0&&<button className="secondary-button" onClick={()=>setReveal(1)}>힌트만 보기</button>}{reveal>=1&&<div className="thought-hint"><small>생각의 방향</small><p>{active.hint}</p></div>}{reveal===1&&<button className="primary-button" onClick={()=>setReveal(2)}>개념 확인하기 <span>→</span></button>}{reveal>=2&&<div className="thought-answer"><small>CONCEPT</small><p>{active.answer}</p><label>내 말로 한 줄 정리<textarea placeholder="내가 이해한 내용을 짧게 적어보세요." /></label></div>}</div></article></section></div>;
}

function NotesPage({ notes, problems }) {
  const entries=Object.entries(notes).filter(([,value])=>value);
  return <div className="page"><PageHeader eyebrow="MY OBSERVATIONS" title="Notes" description="풀이 중 적은 관찰과 해결 후 복기를 한곳에서 다시 봅니다." /><div className="notes-grid">{entries.map(([key,value])=>{const id=key.slice(0,6);const problem=problems.find((item)=>item.id===id);return <article key={key}><span>{id} · {problem?.category||'복기'}</span>{typeof value==='string'?<p>{value}</p>:<><h3>{value.idea||'풀이 복기'}</h3><p>{value.signal||value.reason}</p><small>{value.complexity}</small></>}</article>})}{!entries.length&&<div className="empty-state wide"><span>≡</span><h3>아직 남긴 메모가 없어요.</h3><p>문제 화면의 내 메모와 풀이 후 복기가 자동으로 모입니다.</p></div>}</div></div>;
}
...
export default function App() {
  ...
  const [settings,setSettings]=useState(()=>getLocal('loopin-settings',{theme:'light',editorFontSize:14,focusMinutes:50}));
  const [active,setActive]=useState('today'); const [collapsed,setCollapsed]=useState(false); const [problems,setProblems]=useState([]); const [dailyPlan,setDailyPlan]=useState([]); const [todayMinutes,setTodayMinutes]=useState(profile?.dailyMinutes||120); const [needsDailyTime,setNeedsDailyTime]=useState(false); const [progress,setProgress]=useState({}); const [notes,setNotes]=useState({}); const [selected,setSelected]=useState(null); const [loading,setLoading]=useState(Boolean(profile));

  const loadData=async(activeProfile=profile)=>{setLoading(true);const [loaded,storedProgress,storedNotes,storedDaily]=await Promise.all([loadProblemsByLevel(0),storage.getAll('progress'),storage.getAll('notes'),storage.get('curriculum',todayKey())]);const minutes=storedDaily?.minutes||activeProfile?.dailyMinutes||120;const loadedMastery=calculateConceptMastery(loaded,storedProgress);let daily=(storedDaily?.ids||[]).map((id)=>loaded.find((item)=>item.id===id)).filter(Boolean);if(!daily.length)daily=selectDailyProblems({problems:loaded,progress:storedProgress,mastery:loadedMastery,count:dailyProblemCount(minutes),difficulty:2});setProblems(loaded);setDailyPlan(daily);setTodayMinutes(minutes);setNeedsDailyTime(!storedDaily?.minutes);setProgress(storedProgress);setNotes(storedNotes);setLoading(false);};
  ...
  const mastery=useMemo(()=>calculateConceptMastery(problems,progress),[problems,progress]);
  const completeOnboarding=async(nextProfile)=>{setProfile(nextProfile);setLocal('loopin-profile',nextProfile);await loadData(nextProfile);};
  const updateProgress=async(id,patch)=>{setProgress((current)=>{const next={...current,[id]:{...current[id],...patch}};storage.set('progress',id,next[id]);return next;});};
  const changeTodayMinutes=async(minutes)=>{const nextMinutes=Math.min(720,Math.max(30,Math.round(minutes)));const desired=dailyProblemCount(nextMinutes);const recommendations=selectDailyProblems({problems,progress,mastery,count:Math.max(desired,dailyPlan.length),difficulty:2});const completed=dailyPlan.filter((item)=>['SOLVED','SOLVED_WITH_HINT','MASTERED'].includes(progress[item.id]?.status));const pending=dailyPlan.filter((item)=>!completed.some((done)=>done.id===item.id));const combined=[...completed,...pending,...recommendations].filter((item,index,array)=>array.findIndex((candidate)=>candidate.id===item.id)===index);const nextPlan=combined.slice(0,Math.max(desired,completed.length));setTodayMinutes(nextMinutes);setDailyPlan(nextPlan);setNeedsDailyTime(false);await storage.set('curriculum',todayKey(),{minutes:nextMinutes,ids:nextPlan.map((item)=>item.id),updatedAt:new Date().toISOString()});};
  const onSetting=(key,value)=>setSettings((current)=>({...current,[key]:value}));
  const nextProblem=()=>{if(!selected)return;const sequence=dailyPlan.some((item)=>item.id===selected.id)?dailyPlan:problems;const index=sequence.findIndex((item)=>item.id===selected.id);setSelected(sequence[(index+1)%sequence.length]);};
  ...
  if(selected)return <Suspense fallback={<div className="app-loading"><span className="brand-mark">L</span><p>코드 에디터를 준비하고 있어요.</p></div>}><ProblemSolver key={selected.id} problem={selected} initialProgress={progress[selected.id]} settings={{...settings,onChange:onSetting}} onClose={closeSolver} onProgress={updateProgress} onNext={nextProblem}/></Suspense>;

  let page;
  if(active==='today')page=<TodayPage profile={profile} daily={dailyPlan} todayMinutes={todayMinutes} onTimeChange={changeTodayMinutes} progress={progress} mastery={mastery} onOpen={setSelected} settings={settings}/>;
  if(active==='problems')page=<ProblemsPage problems={problems} progress={progress} onOpen={setSelected}/>;
  if(active==='roadmap')page=<RoadmapPage mastery={mastery}/>;
  if(active==='review')page=<ReviewPage problems={problems} progress={progress} onOpen={setSelected}/>;
  if(active==='theory')page=<LearningPage type="Theory"/>;
  if(active==='ai')page=<LearningPage type="AI"/>;
  if(active==='notes')page=<NotesPage notes={notes} problems={problems}/>;
  if(active==='analytics')page=<AnalyticsPage problems={problems} progress={progress} mastery={mastery}/>;
  if(active==='settings')page=<SettingsPage settings={settings} onSetting={onSetting} onReload={()=>window.location.reload()}/>;
  return <><div className="app-shell"><Sidebar active={active} collapsed={collapsed} onToggle={()=>setCollapsed((value)=>!value)} onNavigate={navigate} profile={profile}/><div className="app-content">{page}{active==='today'&&!needsDailyTime&&<div className="today-time-floating"><DailyTimeControl minutes={todayMinutes} onChange={changeTodayMinutes} compact/></div>}</div></div>{active==='today'&&needsDailyTime&&<DailyTimeSetup defaultMinutes={todayMinutes} onConfirm={changeTodayMinutes}/>}</>;
}
