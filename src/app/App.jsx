...
function ProblemsPage({ problems, progress, onOpen, languageId }) {
...
export default function App() {
  ...
  if(selected)return <Suspense fallback={<div className="app-loading"><span className="brand-mark">L</span><p>코드 에디터를 준비하고 있어요.</p></div>}><ProblemSolver key={`${selected.id}:${settings.learningLanguage}`} problem={selected} languageId={settings.learningLanguage} initialProgress={languageProgress[selected.id]} settings={{...settings,onChange:onSetting}} onClose={closeSolver} onProgress={updateProgress} onNext={nextProblem}/></Suspense>;
  ...
  if(active==='problems')page=<ProblemsPage problems={problems} progress={languageProgress} onOpen={setSelected} languageId={settings.learningLanguage}/>;
  if(active==='roadmap')page=<RoadmapPage mastery={mastery}/>;
  if(active==='review')page=<ReviewPage problems={problems} progress={languageProgress} onOpen={setSelected} languageId={settings.learningLanguage}/>;
  ...
  if(active==='analytics')page=<AnalyticsPage problems={problems} progress={languageProgress} mastery={mastery}/>;
