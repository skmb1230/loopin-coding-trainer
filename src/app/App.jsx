...
import WorkplaceLearning, { WorkplaceTodayCard, useWorkplaceLearning, WORKPLACE_STORAGE_KEY } from '../features/workplace/WorkplaceLearning.jsx';
import { localDayKey } from '../core/workplace/learning.js';
...
function TodayPage({ profile, daily, todayMinutes, onTimeChange, progress, mastery, onOpen, settings, languageId, curriculum, completedSessions, onSessionToggle, workplace, onWords, planDay }) {
...
function ProblemsPage({ problems, storedProgress, onOpen, languageId, level, onLevelChange, loading }) {
  ...
  return <div className="page"><PageHeader eyebrow={`${total} PROBLEMS · ${scopeLabel} · ${getLanguage(languageId).label.toUpperCase()}`} title="문제 탐색" description="선택한 레벨만 지연 로딩합니다. ‘전체 레벨’을 고르면 800문제를 한 번에 검색할 수 있어요." action={<div className="header-stat"><b>{Object.values(progress).filter((item)=>item.status?.startsWith('SOLVED')||item.status==='MASTERED').length}</b><span>solved</span></div>} /><div className="filter-bar"><label className="search-box"><span>⌕</span><input value={query} onChange={(event)=>{setQuery(event.target.value);setPage(1);}} placeholder="문제명 또는 ID 검색" /></label><label>Level<select value={level} onChange={(event)=>onLevelChange(Number(event.target.value))}><option value="-1">전체 레벨 · 800개</option>{problemCountsByLevel.map((count,index)=><option value={index} key={index}>Level {index} · {count}개</option>)}</select></label><label>Category<select value={category} onChange={(event)=>{setCategory(event.target.value);setPage(1);}}>{categories.map((item)=><option key={item}>{item}</option>)}</select></label><label>Status<select value={status} onChange={(event)=>{setStatus(event.target.value);setPage(1);}}><option>전체</option><option value="NOT_STARTED">시작 전</option><option value="TRYING">풀이 중</option><option value="SOLVED">해결</option><option value="SOLVED_WITH_HINT">힌트로 해결</option><option value="FAILED">복습 필요</option><option value="MASTERED">익숙함</option></select></label></div><div className="problem-table"><div className="table-head"><span>상태</span><span>번호</span><span>문제</span><span>유형</span><span>난이도</span><span>예상 시간</span><span>마지막 풀이</span></div>{loading?<div className="empty-table">{scopeLabel} 문제를 불러오는 중입니다.</div>:visible.map((problem)=>{const record=progress[problem.id];const state=record?.status||'NOT_STARTED';return <button className="table-row" key={problem.id} onClick={()=>onOpen(problem)}><span><i className={`table-status ${state.toLowerCase()}`}>{['SOLVED','MASTERED'].includes(state)?'✓':state==='FAILED'?'!':'·'}</i>{statusLabels[state]}</span><span>{getDisplayProblemId(problem.id,languageId)}</span><span><b>{problem.title}</b><small>Level {problem.level} · {problem.concepts.join(' · ')}</small></span><span>{problem.category}</span><span>{'●'.repeat(problem.difficulty)}<i>{'●'.repeat(5-problem.difficulty)}</i></span><span>{problem.estimatedMinutes}분</span><span>{record?.lastAttempt?new Intl.DateTimeFormat('ko-KR').format(new Date(record.lastAttempt)):'—'} <b>→</b></span></button>})}{!loading&&!filtered.length&&<div className="empty-table">조건에 맞는 문제가 없어요.</div>}</div>{!loading&&filtered.length>pageSize&&<nav className="table-pagination" aria-label="문제 목록 페이지"><button disabled={safePage===1} onClick={()=>setPage((value)=>Math.max(1,value-1))}>← 이전</button><span>{safePage} / {pageCount} · {filtered.length}문제</span><button disabled={safePage===pageCount} onClick={()=>setPage((value)=>Math.min(pageCount,value+1))}>다음 →</button></nav>}</div>;
}
...
function LearningPage({ type, languageId }) {
  ...
  const topics=config.topics; const [active,setActive]=useState(topics[0]); const [reveal,setReveal]=useState(0); const [learningProgress,setLearningProgress]=useState(()=>getLocal('loopin-learning-progress',{}));
...
function SettingsPage({ settings, onSetting, onReload }) {
  ...
  const resetLearningHistory=async()=>{if(!window.confirm('이론·회의 용어 학습 기록과 오늘 세션 완료 상태를 초기화할까요? 이 작업은 되돌릴 수 없어요.'))return;await Promise.all([storage.clear('sessions'),storage.clear('curriculum')]);localStorage.removeItem('loopin-learning-progress');localStorage.removeItem(WORKPLACE_STORAGE_KEY);onReload();};
  const clearLevel=async()=>{if(!window.confirm(`Level ${resetLevel}의 진도·코드·메모를 두 언어 모두 초기화할까요?`))return;const matches=(key)=>getProblemLevel(getProblemIdFromStorageKey(key))===resetLevel;await Promise.all(['progress','code','notes'].map((store)=>storage.removeWhere(store,(key)=>matches(key))));onReload();};
  return <div className="page settings-page"><PageHeader eyebrow="PREFERENCES & DATA" title="Settings" description="집중 환경과 로컬 학습 데이터를 관리합니다." /><section><h2>학습 언어</h2><div className="setting-row"><div><strong>기본 풀이 언어</strong><p>Today·Problems·Theory·복습 기록이 선택한 언어로 전환됩니다.</p></div><select value={settings.learningLanguage} onChange={(event)=>onSetting('learningLanguage',event.target.value)}>{getAvailableLanguages().map((language)=><option value={language.id} key={language.id}>{language.label}</option>)}</select></div>{settings.learningLanguage==='java'&&<JavaRuntimeStatus />}</section><section><h2>화면과 에디터</h2><div className="setting-row"><div><strong>화면 테마</strong><p>오래 봐도 편안한 화면을 선택하세요.</p></div><div className="toggle-group"><button className={settings.theme==='light'?'active':''} onClick={()=>onSetting('theme','light')}>Light</button><button className={settings.theme==='dark'?'active':''} onClick={()=>onSetting('theme','dark')}>Dark</button></div></div><div className="setting-row"><div><strong>에디터 글자 크기</strong><p>코드 에디터에만 적용됩니다.</p></div><select value={settings.editorFontSize} onChange={(event)=>onSetting('editorFontSize',Number(event.target.value))}>{[13,14,15,16,18].map((item)=><option value={item} key={item}>{item}px</option>)}</select></div><div className="setting-row"><div><strong>기본 집중 세션</strong><p>오늘의 학습 세션을 나누는 기준입니다.</p></div><select value={settings.focusMinutes} onChange={(event)=>onSetting('focusMinutes',Number(event.target.value))}>{[25,40,50,60,90].map((item)=><option value={item} key={item}>{item}분</option>)}</select></div></section><section><h2>Backup</h2><div className="setting-row"><div><strong>데이터 내보내기</strong><p>진도, 코드, 메모, 이론과 회의 용어의 계획·복습 기록을 JSON 파일로 저장합니다.</p></div><button className="secondary-button" onClick={exportData}>JSON 내보내기</button></div><div className="setting-row"><div><strong>데이터 가져오기</strong><p>다른 PC에서 저장한 loopin 백업을 복구합니다.</p></div><label className="secondary-button file-button">JSON 가져오기<input type="file" accept="application/json" onChange={importData} /></label></div></section><section className="danger-zone"><h2>Reset</h2><div className="setting-row"><div><strong>특정 Level 초기화</strong><p>선택한 레벨의 진도·코드·메모만 지웁니다.</p></div><div className="inline-reset"><select value={resetLevel} onChange={(event)=>setResetLevel(Number(event.target.value))}>{problemCountsByLevel.map((_,level)=><option value={level} key={level}>Level {level}</option>)}</select><button onClick={clearLevel}>선택 Level 초기화</button></div></div><div className="reset-actions"><button onClick={()=>reset('progress','문제 진행 상황')}>문제 진행 초기화</button><button onClick={resetLearningHistory}>학습 기록 초기화</button><button onClick={()=>reset('code','작성 코드')}>문제 코드 초기화</button><button onClick={async()=>{if(window.confirm('프로필을 포함한 전체 데이터를 초기화할까요?')){for(const store of ['progress','code','notes','sessions','curriculum'])await storage.clear(store);clearLocalLearningData();onReload();}}}>전체 데이터 초기화</button></div></section></div>;
}
...
export default function App() {
  ...
  const [settings,setSettings]=useState(()=>{
    ...
    return { theme:'light', editorFontSize:14, focusMinutes:profile?.focusMinutes||50, ...saved, learningLanguage:saved.learningLanguage || profile?.learningLanguage || 'javascript' };
  });
  ...
  const workplace = useWorkplaceLearning();
  ...
  const loadData=async(activeProfile=profile, languageOverride)=>{
    ...
    progressRef.current=storedProgress;setPlanDay(requestedDay);dailySnapshot.current={minutes,plan:daily,sessions:storedDaily?.completedSessions||[]};
    setProblems(loaded);setBrowseLevel(curriculum.currentLevel);setBrowseProblems(loaded.filter((item)=>item.level===curriculum.currentLevel));setDailyPlan(daily);setTodayMinutes(minutes);setNeedsDailyTime(!storedDaily?.minutes);setDailySessionDone(storedDaily?.completedSessions||[]);setProgress(storedProgress);setNotes(storedNotes);setLoading(false);
    } catch(error) {
  ...
  useEffect(()=>{
    const checkDay=()=>{
      if(!profile||selected||loading||loadError||localDayKey(new Date())===planDay)return;
  ...
  },[profile,selected,loading,loadError,planDay,settings.learningLanguage]);
  useEffect(()=>{if(!profile||active!=='problems')return;let current=true;setBrowseLoading(true);const request=browseLevel===-1?loadProblemsByLevels([0,1,2,3,4,5]):loadProblemsByLevel(browseLevel);request.then((items)=>{if(current){setBrowseProblems(items);setProblems((existing)=>[...existing,...items].filter((problem,index,array)=>array.findIndex((candidate)=>candidate.id===problem.id)===index));}}).finally(()=>{if(current)setBrowseLoading(false);});return()=>{current=false;};},[active,browseLevel,profile]);
  ...
  useEffect(()=>{if(!profile||problems.some((problem)=>problem.level===curriculum.currentLevel))return;let current=true;loadProblemsByLevel(curriculum.currentLevel).then((items)=>{if(current)setProblems((existing)=>[...existing,...items].filter((problem,index,array)=>array.findIndex((candidate)=>candidate.id===problem.id)===index));});return()=>{current=false;};},[curriculum.currentLevel,problems,profile]);
  const completeOnboarding=async(nextProfile)=>{sessionStorage.setItem(languageChoiceKey(),'done');setNeedsLanguageChoice(false);setProfile(nextProfile);setSettings((current)=>({...current,learningLanguage:nextProfile.learningLanguage||'javascript',focusMinutes:nextProfile.focusMinutes||current.focusMinutes}));setLocal('loopin-profile',nextProfile);await loadData(nextProfile,nextProfile.learningLanguage||'javascript');};
  ...
  const saveDaily=(transform)=>{
    ...
    const write=dailyWrites.current.catch(()=>{}).then(async()=>{
      ...
      await storage.set('curriculum',key,{minutes:next.minutes,ids:next.plan.map((item)=>item.id),completedSessions:next.sessions,level:curriculum.currentLevel,targetDifficulty:curriculum.targetDifficulty,updatedAt:new Date().toISOString()});
  ...
  if(loadError)return <div className="app-loading"><p role="alert">{loadError}</p><button className="secondary-button" onClick={()=>loadData()}>다시 불러오기</button></div>;
  ...
  let page;
  if(active==='today')page=<TodayPage profile={profile} daily={dailyPlan} todayMinutes={todayMinutes} onTimeChange={changeTodayMinutes} progress={languageProgress} mastery={mastery} onOpen={setSelected} settings={settings} languageId={settings.learningLanguage} curriculum={curriculum} completedSessions={dailySessionDone} onSessionToggle={toggleDailySession} workplace={workplace} onWords={()=>navigate('words')} planDay={planDay}/>;
  ...
  if(active==='words')page=<WorkplaceLearning learning={workplace}/>;
  ...
  return <><div className="app-shell"><Sidebar active={active} collapsed={collapsed} onToggle={()=>setCollapsed((value)=>!value)} onNavigate={navigate} profile={profile} languageId={settings.learningLanguage} currentLevel={curriculum.currentLevel}/><div className="app-content">{dailyError&&<div className="word-inline-error app-save-alert" role="alert">{dailyError}</div>}{page}{active==='today'&&!needsDailyTime&&<div className="today-time-floating"><DailyTimeControl minutes={todayMinutes} onChange={changeTodayMinutes} compact/></div>}</div></div>{needsLanguageChoice?<LanguageStartSetup selectedLanguage={settings.learningLanguage} onConfirm={confirmStartLanguage}/>:active==='today'&&needsDailyTime&&<DailyTimeSetup defaultMinutes={todayMinutes} onConfirm={changeTodayMinutes}/>}</>;
}
