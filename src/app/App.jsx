...
function NotesPage({ notes, problems }) {
  ...
  return <div className="page"><PageHeader eyebrow="MY OBSERVATIONS" title="Notes" description="풀이 중 적은 관찰과 해결 후 복기를 한곳에서 다시 봅니다." /><div className="notes-grid">{entries.map(([key,value])=>{const id=key.slice(0,6);const problem=problems.find((item)=>item.id===id);return <article key={key}><span>{id} · {problem?.category||'복기'}</span>{typeof value==='string'?<p>{value}</p>:<><h3>{value.idea||'풀이 복기'}</h3><p>{value.signal||value.reason}</p><small>{value.type==='struggle'?'막힘 기억 카드':value.complexity}</small></>}</article>})}{!entries.length&&<div className="empty-state wide"><span>≡</span><h3>아직 남긴 메모가 없어요.</h3><p>문제 화면의 내 메모와 풀이 후 복기가 자동으로 모입니다.</p></div>}</div></div>;
}
