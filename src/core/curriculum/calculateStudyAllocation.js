const ratiosByStage = {
  beginner: { problems: 55, theory: 20, ai: 15, career: 5, review: 5 },
  intermediate: { problems: 65, theory: 15, ai: 10, career: 5, review: 5 },
  practical: { problems: 70, theory: 10, ai: 5, career: 5, review: 10 },
};

const allocationKeys = ['problems', 'theory', 'ai', 'career', 'review'];
const remainderPriority = ['review', 'problems', 'theory', 'ai', 'career'];
...
export function calculateStudyAllocation(totalMinutes, stage = 'beginner', customRatios) {
  ...
  const ratios = minutes < 120 && (baseRatios.career || 0) > 0
    ? { ...baseRatios, ai: (baseRatios.ai || 0) + baseRatios.career, career: 0 }
    : baseRatios;
...
export function buildStudySessions(allocation, focusMinutes = 50, date = new Date(), languageLabel = 'JavaScript') {
  ...
  for (const key of allocationKeys) {
    ...
    while (remaining > 0) {
      ...
      sessions.push({ id: `${key}-${sessions.length}`, type: key, title: labels[key], duration, done: false, track: key === 'career' ? careerTrack.id : undefined });
      remaining -= duration;
    }
  }
  return sessions;
