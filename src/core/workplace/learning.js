...
/** Return the user's local calendar date, never the UTC date slice. */
export function localDayKey(now) {
  const date = typeof now === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(now) ? dateFromDay(now) : now instanceof Date ? new Date(now.getTime()) : typeof now === 'number' || typeof now === 'string' ? new Date(now) : null;
  if (!date || !Number.isFinite(date.getTime())) throw new TypeError('A valid explicit date is required.');
  return `${String(date.getFullYear()).padStart(4, '0')}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`;
}

/** Calendar arithmetic survives 23-hour and 25-hour daylight-saving days. */
